...
class Tab: NSObject {
    ...
    init(title: String, url: URL, customCss: [URL], customJs: [URL], customCookies: [Config.Schema.Cookie], basicAuthUser: String, basicAuthPassword: String, userAgent: String?) {
        let configuration = WKWebViewConfiguration()
        Browser.global.customCss(configuration, urls: customCss)
        Browser.global.customJs(configuration, urls: customJs)
        Browser.global.customCookies(configuration, cookies: customCookies)
